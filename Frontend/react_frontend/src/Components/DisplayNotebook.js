...
const DisplayNotes = props => {
    ...
    react.useEffect(() => {
        const get_notes = async () => {
            const notes = await axios.get(`http://127.0.0.1:5000/get_notes_from_notebook?note_book_name=${props.notebook_name}`)
                ...
                return (
                    <div className={`card ${(props.task_view_flag) ? 'col-sm-8' : 'col-sm-5'} mx-auto my-3 text-center`}>
                        ...
                        <div style={greyandpurple} className='card-footer'>
                            <a id={note._id} onClick={move_to_archive_click_handler} href='#' className="text-decoration-none fw-bold mx-5 text-success">Archive</a>
        ...
        get_notes()
    }, [text_editor_display, props.notebook_name, props.task_view_flag])
        ...
        if (text_editor_display) {
            if (open_editor.flag) {
                if (open_editor.parti_note.data.notes_jsx !== editor_data.editor_notes || open_editor.parti_note.data.note_title !== editor_data.title) {
                    axios.put(`http://127.0.0.1:5000/update_note_from_notebook/${open_editor.id}?note_book_name=${props.notebook_name}`, editor_data)
                }
                set_open_editor({ ...open_editor, flag: 0 })
            }
            else {
                axios.post(`http://127.0.0.1:5000/post_notes_to_notebook?note_book_name=${props.notebook_name}`, editor_data)
            }
        }
    ...
    const open_note_click_handler = async e => {
        const parti_note = await axios.get(`http://127.0.0.1:5000/get_note_from_notebook_by_id/${e.target.id}?note_book_name=${props.notebook_name}`)
    ...
    const move_to_trash_click_handler = async e => {
        let parti_note = await axios.get(`http://127.0.0.1:5000/get_note_from_notebook_by_id/${e.target.id}?note_book_name=${props.notebook_name}`)
        try {
            parti_note = { title: parti_note.data.note_title, notes: parti_note.data.notes_jsx }
        } catch { parti_note = { title: '', notes: '' } }
        axios.post(`http://127.0.0.1:5000/post_to_bin?notebook_name=${props.notebook_name}`, parti_note)
        axios.delete(`http://127.0.0.1:5000/delete_note_from_notebook/${e.target.id}?note_book_name=${props.notebook_name}`)
    ...
    const move_to_archive_click_handler = async e => {
        ...
        axios.post(`http://127.0.0.1:5000/post_to_archive?notebook_name=${props.notebook_name}`, parti_note)
        axios.delete(`http://127.0.0.1:5000/delete_note_from_notebook/${e.target.id}?note_book_name=${props.notebook_name}`)
        set_text_editor_display(1)
        set_text_editor_display(0)
    }
    ...
    return (
        <div className="container-sm">
            <div className='row'>
                <div className='text-center '><label style={gold} className='fst-italic col-sm-12  fs-2'>{props.notebook_name}</label></div>
                <div className='text-center '> <a href='#' onClick={editor_display_click_handler} className='text-decoration-none fs-3 mx-5'>{(text_editor_display) ? 'I am done.' : 'Start Noting.....'}</a></div>
            </div>
